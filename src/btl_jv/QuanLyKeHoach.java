...
//    }
    ...
    public JTextField getTxtTieuDe() throws Exception {
        if (txtTieuDe.getText().equals("")) {
            throw new Exception("Không được để trống tiêu đề");
        }
        return txtTieuDe;
    }
    ...
    private void btnSuaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSuaActionPerformed
        ...
        if(dongchon != -1){
            try {
            String tieuDeHienTai = dskh.get(dongchon).getTieuDe(); // Lấy tiêu đề hiện tại của kế hoạch
            String tieuDeMoi = getTxtTieuDe().getText(); // Lấy tiêu đề mới từ giao diện

            if (!tieuDeMoi.equals(tieuDeHienTai)) {
                JOptionPane.showMessageDialog(this, "Không được thay đổi tiêu đề", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
                return; 
            }                
                kh.setTieuDe(tieuDeHienTai);
                kh.setTrangThai(cbxTrangThai.getSelectedItem().toString());
                kh.setNgayBatDau(iniNgayBD());
                kh.setNgayKetThuc(iniNgayKT());
                kh.setHanDangKy(iniHanDK());
                kh.setHanNopBC(iniHanNop());
                kh.setNgayDiTT(iniNgayDi());
                kh.setNgayHetTT(iniNgayHet());
                kh.setNoidung(getTxtNoiDung().getText());                
